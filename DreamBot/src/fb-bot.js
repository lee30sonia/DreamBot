  ...
  for (let i = 0; i < messaging_events.length; i++) {
    ...
    if (event.message && event.message.text) {
      ...
      if (event.message.quick_reply)
      {
         ...
         if (qr.payload=="take_test")
            takeTest(sender);
         else if (text=="I remember!")
            response_rem(sender,qr.payload,true);
         else if (text=="I forgot QQ")
            response_rem(sender,qr.payload,false);
         else if (text=="It's correct!")
            word_correct(sender,qr.payload);
         else if (text=="It's wrong QQ" || text=="ok let's go")
            word_wrong(sender,qr.payload);
      }
    ...
    {
      if (event.postback.payload=="cam")
         addWord(sender,event.postback.title,"cam");
      else if (event.postback.payload=="oxf")
         addWord(sender,event.postback.title,"oxf");
      else if (event.postback.title=="Search!")
      {
...
function response_rem(who,word,rem) {
   ...
   mongo.connect(mongoUrl, function(err, db) {
      ...
      db.collection(col).findOne({voc:word}, function(err,res){
         if (err) throw err;
         if (res.dic=="oxf") oxford(word,who);
         else if (res.dic=="cam") cambridge(word,who);
         db.close();
      });
   });
   setTimeout(function(){
      if (rem)
         sendTextMessageWithQR(who,"Did you remember it correctly?",[{
           "content_type":"text",
           "title":"It's correct!",
           "payload":word
           },{
         ...
         sendTextMessageWithQR(who,"Ready for the next question?",[{
           "content_type":"text",
           "title":"ok let's go",
           "payload":word
           }]);
